...
type VideoBaseInfo struct {
	...
	TotalCapacity     int64   // 存储池总容量大小，单位 MiB
...
type VideoDataInfo struct {
	...
	BucketNum    int   // 桶数量
	ObjNum       int64 // 安全水位能写入的总对象数量，达到该数量后需要 边写边删
	ObjNumPC     int64 // 安全水位每路视频能写入的对象数量
	ObjNumPD     int64 // 每天需要写入的对象数量，码流+对象大小 计算得出
	ObjNumPCPD   int64 // 每路视频、每天，需要写入的对象数量
	TimeInterval int   // 一路视频中，没个视频对象产生的实际间隔，单位：秒
	MaxWorkers   int16 // 一路视频中，最大处理并行数
...
type VideoCustomizeInfo struct {
	...
	ObjIdxWidth       int64  // 对象名称序号长度， 3=>001
	ObjIdxStart       int64  // 对象处理 序号起始值
}
