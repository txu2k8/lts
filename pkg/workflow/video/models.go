...
type VideoBaseInfo struct {
	...
	FileReader io.ReadSeeker // 文件Reader
	Segments   int           // 追加写模式下，一个对象追加分片次数

	TotalCapacity     uint64  // 存储池总容量大小，单位 byte
	SafeWaterLevel    float32 // 安全水位，即数据写入存储池的数据量最大不超过总容量的百分比，例如 90%=0.9
	SafeWaterCapacity uint64  // 安全水位存储池容量大小，单位 byte; SafeWaterCapacity=TotalCapacity*SafeWaterLevel

	Appendable       bool // 追加写模式
	DisableMultipart bool // 非多段上传，与追加写互斥
...
type VideoDataInfo struct {
	...
	TimeInterval        int // 一路视频中，每个视频对象产生的实际间隔，单位：秒
	SegmentTimeInterval int // 一路视频中，每个视频对象分片产生的实际间隔，单位：秒
